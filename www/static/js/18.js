...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = {
	    ...
	    route: {
	        data: function data() {
	            if (!this.$route.params.id) return;
	            this.$http.get('/kodo/article/detail/id/' + this.$route.params.id).then(function (ret) {
	                this.title = ret.data.title;
	                this.editorContent = decodeURIComponent(ret.data.content);
	            });
	        }
	    ...
	    methods: {
	        ...
	        markRender: function markRender() {
	            //实时渲染markdown
	            _prismjs2.default.highlightAll(false);
	        },
	...
	function create(v) {
	    v.$http.post("/kodo/article/add", {
	        title: v.title,
	        content: v.editorContent,
	        bg: (0, _stringify2.default)(v.badges)
	    }).then(function (ret) {
	        alert('发射成功');
	    });
	}
